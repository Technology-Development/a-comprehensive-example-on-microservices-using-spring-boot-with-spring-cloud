...
Logger logger = LoggerFactory.getLogger(CheckinController.class);
	...
	public ResponseEntity<CheckinResponse> saveNewCheckin(@RequestBody Checkin checkin) {
		CheckinResponse checkinResponse = checkinService.saveNewCheckin(checkin);
		return new ResponseEntity<CheckinResponse>(checkinResponse, HttpStatus.CREATED);
	}
	
	/*
	 * API to get a check-in details of a booking by check-in id
	 * 
	...
	public ResponseEntity<CheckinResponse> getCheckinById(@PathVariable Long id) {
		...
		return new ResponseEntity<CheckinResponse>(checkinResponse, HttpStatus.OK);
	}
	
	/*
	 * API to find a checkin details of a booking by booking id
	 * 
